...
const from = (f) => {
    return aeither(new Promise(async (resolve) => {
        try {
            const value = await f();
            resolve(index.either(index.right(value)));
        }
        catch (error) {
            resolve(index.either(index.left(error)));
        }
    }));
};
/**
 ...
 * @returns Promise<R>
 */
const unwrap = async (either) => index.unrightOr(index$1.throws, await either);
/**
 ...
 * @returns Promise<R>
 */
const unrightOr = async (or, either) => index.unrightOr(or, await either);
/**
 ...
 * @returns Promise<L>
 */
const unleftOr = async (or, either) => index.unleftOr(or, await either);
/**
 * Maps the right value if present with the mapping function.
 *
 * @returns IAEither<L, B>
 */
const map = async (f, either) => index.either(index.right(f(index.unwrap(await either))));
/**
 ...
 * @returns IAEither<L, B>
 */
const bind = async (f, either) => {
    try {
        return index.either(index.right((await f(index.unwrap(await either)))));
    }
    catch (error) {
        return index.either(index.left(error));
    }
};
/**
...
exports.aeither = aeither;
exports.bind = bind;
exports.from = from;
exports.map = map;
exports.unleft = unleft;
exports.unleftOr = unleftOr;
exports.unright = unright;
exports.unrightOr = unrightOr;
