...
const channel = (buffer = []) => {
    ...
    const put = (value) => {
        ...
        const taker = buffer[0] && deferred.isDeferred(buffer[0]) ? buffer[0] : null;
        if (taker) {
            buffer.splice(0, 1);
    ...
    const take = async () => {
        ...
        if (next && deferred.isDeferred(next)) {
            return next.value;
        }
        else if (next) {
            return next;
