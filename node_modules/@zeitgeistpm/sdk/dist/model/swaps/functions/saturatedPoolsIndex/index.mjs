...
import { isIndexerContext, isRpcContext } from '../../../../context/types.mjs';
import { parseAssetId, getIndexOf, IOZtgAssetId, fromPrimitive } from '../../../../primitives/assetid.mjs';
import { ZTG } from '../../../../primitives/ztg.mjs';
import '@zeitgeistpm/utility/dist/pfunc';
...
import { rpcMarket } from '../../../markets/market.mjs';
import 'rxjs';
...
const indexer = async (ctx, pools) => {
    ...
    const byPool = (await Promise.all(pools.map(async (pool) => {
        ...
        const assets = pool.weights
            ...
            const percentage = Math.round(new Decimal(weight.len).dividedBy(pool.totalWeight).mul(100).toNumber());
        ...
        const liquidity = assets.reduce((total, asset) => {
            ...
            return total.plus(new Decimal(asset.price.div(ZTG)).mul(new Decimal(asset.amount)));
        }, new Decimal(0));
...
const rpc = async (ctx, pools) => {
    const byPool = await Promise.all(pools.map(async (pool) => {
        ...
        let liquidity = pool.assets.reduce((total, asset, index) => {
            ...
            return total.plus(new Decimal(price.toNumber()).div(ZTG).mul(amountInPool.toNumber()));
        }, new Decimal(0));
...
export { indexer, rpc, saturatedPoolsIndex };
