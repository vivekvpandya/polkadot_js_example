...
import { range, project, zip } from '@zeitgeistpm/utility/dist/array';
import { asBlocks, asMs, isBlocks, asBlock } from '@zeitgeistpm/utility/dist/time';
import { Decimal } from 'decimal.js';
import { Observable } from 'rxjs';
import { isFullContext, isIndexerContext } from '../../../../context/types.mjs';
import { IOAssetId, getIndexOf } from '../../../../primitives/assetid.mjs';
import { now } from '../../../time/functions/now/index.mjs';
...
const poolPrices = async (context, query) => {
    const data = isFullContext(context) || isIndexerContext(context)
...
const rpcPoolPrices = async (ctx, query) => {
    ...
    const prices = await Promise.all(assets.map(async (asset) => {
        ...
        console.log('BAAR');
        return zip(blocks, prices.map(price => new Decimal(price.toString())));
    }));
    return prices;
};
/**
...
const indexerPoolPrices = async (context, query) => {
    ...
    for (const record of historicalAssets) {
        ...
        prices[index] = [
            ...(prices[index] || []),
            [record.blockNumber, new Decimal(record.newPrice * 10 ** 10)],
        ];
    }
    return prices;
};
        ...
        const unsub = Promise.all([
            ...
            return ctx.api.rpc.chain.subscribeFinalizedHeads(async (header) => {
                ...
                const prices = await Promise.all(assets.map(async (asset) => {
                    ...
                    return [block, new Decimal(price.toString())];
                }));
                sub.next(prices);
            });
        });
...
export { observePoolPrices$, poolPrices };
