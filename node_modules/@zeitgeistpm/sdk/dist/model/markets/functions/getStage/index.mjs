...
import { getStatus, getDeadlines, timespanOf, getReportedAt } from '../../market.mjs';
 ...
 */
const getStage = async (ctx, market, providedTime) => {
    ...
    if (status === 'Disputed') {
        ...
        if (!report.isEmpty) {
            ...
            const reportedAtTimestamp = Time.blockDate(time, reportedAtBlock).getTime();
            const correctionPeriod = O.tryCatch(() => ctx.api.consts.authorized['correctionPeriod'].toNumber()).unwrapOr(7200);
